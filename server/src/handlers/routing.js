...
// src/handlers/routing.js
// Feature 2 — Smart Routing + Impact Intelligence Module
// Combines routing decision engine with financial breakdown and sustainability scores.
...
export async function routeItem(req, res) {
  try {
    const {
      ...
      demandLevel,
      returnReason,
      // Legacy inputs (backwards compat)
      grade,
      trustScore,
      estimatedValue,
      localDemandScore,
    ...
    // Resolve inputs — prefer explicit fields, fall back to legacy
    const resolvedScore =
      conditionScore !== undefined ? Number(conditionScore)
      : trustScore   !== undefined ? Number(trustScore)
      : gradeToScore(grade);
    ...
    const resolvedDemand =
      demandLevel !== undefined ? demandLevel
      : localDemandScore >= 70  ? "high"
      : localDemandScore >= 40  ? "medium"
      : "low";

    const resolvedRepair = Number(estimatedRepairCost) || 0;
    const resolvedCat    = category || "General";

    // 1️⃣  Routing decision engine
    const engineResult = calculateRoute({
      conditionScore:     resolvedScore,
      category:           resolvedCat,
      originalPrice:      resolvedPrice,
      estimatedRepairCost: resolvedRepair,
      demandLevel:        resolvedDemand,
      returnReason:       returnReason || "",
    });

    // 2️⃣  Impact intelligence (financial + sustainability)
    ...
    // 3️⃣  All recovery values (for UI comparison bars)
    const recoveryValues = calculateRecoveryValue({
      conditionScore:     resolvedScore,
      originalPrice:      resolvedPrice,
      estimatedRepairCost: resolvedRepair,
      demandLevel:        resolvedDemand,
    ...
    // 4️⃣  Bedrock AI summary
    let aiSummary = null;
    try {
      const prompt = buildAISummaryPrompt({
        productName:    productName || resolvedCat,
        category:       resolvedCat,
        conditionScore: resolvedScore,
        routeLabel:     engineResult.routeLabel,
      ...
      aiSummary = await invokeClaudeText(prompt, {
        maxTokens: 200,
        systemPrompt:
          "You are a sustainability advisor for ReCircle. Write warm, specific, encouraging summaries for sellers. Never use markdown. Plain text only.",
      });
      aiSummary = aiSummary?.trim() || null;
    } catch (err) {
      console.warn("Bedrock AI summary unavailable:", err.message);
    }

    // 5️⃣  Persist to DynamoDB — RoutingHistory schema
    const decisionId = uuidv4();
    const now        = new Date().toISOString();

    const record = {
      // Core RoutingHistory fields
      itemId:     itemId || "unlinked",
      route:      engineResult.route,
      timestamp:  now,
      confidence: engineResult.confidence,
    ...
    await dynamo.send(new PutCommand({ TableName: TABLES.ROUTING, Item: record }));

    // 6️⃣  Update item status if itemId provided
    if (itemId && itemId !== "unlinked") {
      try {
        await dynamo.send(
          new UpdateCommand({
    ...
    return res.status(201).json({
      ...
      // Recovery comparison
      recoveryValues,
      expectedRecoveryValue: impact.financial.netRecovery,
    });
  } catch (err) {
    console.error("routeItem error:", err);
    return res.status(500).json({ error: "Routing decision failed." });
  ...
  try {
    ...
    return res.json({
      ...
      metrics: {
        ...
        totalCarbonSavedKg:  (agg.totalItemsGraded || 120) * 8.5,
        totalWastePreventedKg: (agg.totalItemsGraded || 120) * 1.1,
        avgCircularityScore: 84,
      },
    });
  } catch (err) {
    console.error("getRoutingMetrics error:", err);
