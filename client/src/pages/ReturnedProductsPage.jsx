...
// // A listing counts as "today" if it was published on the current calendar date
// const isPublishedToday = (publishedAt) => {
...
// // Baseline demo data — shown alongside any real listings published today
// const demoReturns = [
//   {
//     id: 0,
...
//   // Poll the in-memory publishedListings store for items published today
//   useEffect(() => {
//     const refresh = () => {
//       const todaysLive = publishedListings.filter((listing) =>
//         isPublishedToday(listing.publishedAt)
//       );
//       setLiveReturns(todaysLive);
//     };
//     refresh();
...
//   const allReturns = [...liveReturns, ...demoReturns];
//   const totalToday = allReturns.length;
...
//                       src={product.image}
//                       alt={product.title}
//                       className={`h-full w-full ${product.isLive ? "object-cover" : "object-contain"}`}
//                     />
//                     <div className="absolute top-2 left-2">
//                       <GradeBadge grade={product.grade} />
//                     </div>
//                     {product.isLive && (
//                       <div className="absolute bottom-2 left-2 text-[10px] xl:text-xs bg-emerald-500 text-white font-bold px-2 py-1 rounded-full">
//                         🆕 Just Listed
//                       </div>
//                     )}
...
//                       </div>
//                     </div>
//                     <Link to={product.isLive ? "/recircle/buy" : `/product/${product.id}`}>
...
import { GB_CURRENCY } from "../utils/constants";
import { getTodaysListings } from "../utils/CallApi";
...
const ReturnedProductsPage = () => {
  ...
  // Fetch today's published listings from the database and poll every 30s
  useEffect(() => {
    const fetchToday = async () => {
      try {
        const data = await getTodaysListings();
        // Normalise DB fields to the shape the card UI expects
        const normalised = (data.listings || []).map((l) => ({
          id: l.listingId,
          title: l.productName,
          image: l.photoUrl || "/images/product_0_small.jpg",
          category: l.category,
          grade: l.grade,
          trust: l.trustScore,
          price: l.price,
          oldPrice: l.originalPrice || l.price * 2,
          reason: l.returnReason || "Listed via Sell page",
          returnedAt: new Date(l.createdAt).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
          route: l.routeLabel || "AI Verified → ReCircle Zone",
          isLive: true,
        }));
        setListings(normalised);
        setError(null);
      } catch (err) {
        console.error("Failed to load today's listings:", err);
        setError("Could not load today's listings. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const totalToday = listings.length;
  const avgDiscount =
    totalToday > 0
      ? Math.round(
          listings.reduce(
            (acc, p) => acc + (1 - p.price / (p.oldPrice || p.price * 2)) * 100,
            0
          ) / totalToday
        )
      : 0;
    ...
    <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 min-h-screen relative">
      <div className="min-w-[1000px] max-w-[1500px] m-auto p-6 relative">
        ...
        <div className="relative z-10">
          ...
          {loading && (
            <div className="text-white/50 text-center py-16">Loading today's listings…</div>
          )}
          ...
          {!loading && !error && listings.length === 0 && (
            <div className="text-white/50 text-center py-16">
              No items published on sale today yet. Check back soon!
            </div>
          )}
              ...
              {listings.map((product, i) => (
                ...
                >
                  <GlassCard className="p-4 h-full flex flex-col" delay={0}>
                    ...
                    <div className="text-xs xl:text-sm text-white/50 mb-1">
                      Return reason:{" "}
                      <span className="text-white/70">{product.reason}</span>
                    </div>
                    <div className="text-xs xl:text-sm text-emerald-400 mb-3">
                      {product.route}
                    </div>
