...
import RoutingDecisionCard from "../components/RoutingDecisionCard";
import ImpactInsightsPanel from "../components/ImpactInsightsPanel";

// ── Network visualisation nodes ───────────────────────────────────────────────
const nodes = [
  { x: 60,  y: 60,  label: "Return Hub"        },
  { x: 240, y: 40,  label: "Processing Center"  },
  { x: 360, y: 120, label: "Resale Hub"         },
...
const PRESETS = [
  { label: "Like-New Electronics", emoji: "🎧",
    values: { conditionScore: 95, category: "Electronics",   originalPrice: 120, estimatedRepairCost: 0,   demandLevel: "high",   returnReason: "Changed mind",  productName: "Sony WH-1000XM5 Headphones" } },
  { label: "Wrong-Size Trainers",  emoji: "👟",
    values: { conditionScore: 92, category: "Shoes",         originalPrice: 80,  estimatedRepairCost: 0,   demandLevel: "medium", returnReason: "Wrong size",     productName: "Nike Air Max 270" } },
  { label: "Damaged Laptop",       emoji: "💻",
    values: { conditionScore: 55, category: "Electronics",   originalPrice: 700, estimatedRepairCost: 120, demandLevel: "high",   returnReason: "Screen cracked", productName: "Dell XPS 13 Laptop" } },
  { label: "Worn Clothing",        emoji: "👕",
    values: { conditionScore: 35, category: "Clothing",      originalPrice: 40,  estimatedRepairCost: 0,   demandLevel: "low",    returnReason: "Doesn't fit",    productName: "Levi's Denim Jacket" } },
];

const CATEGORIES  = ["Electronics","Clothing","Shoes","Fashion","Home & Kitchen","Baby Products","Sports","General"];
const DEMAND_OPTS = ["high","medium","low"];
...
const RoutingPage = () => {
  const [form, setForm] = useState({
    conditionScore: 80, category: "Electronics", originalPrice: 150,
    estimatedRepairCost: 20, demandLevel: "medium", returnReason: "Changed mind",
    productName: "",
  });
  const [gradingResult, setGradingResult] = useState(null);
  ...
  const applyPreset = (preset) => { setForm(preset.values); setGradingResult(null); setDecision(null); };

  const runEngine = () => {
    setRunning(true); setGradingResult(null); setDecision(null);
    setTimeout(() => {
      setGradingResult({ itemId: null, grading: { trustScore: form.conditionScore } });
      setRunning(false);
    }, 600);
  };
      ...
      <div className="min-w-[1000px] max-w-[1500px] m-auto p-6 relative">
        ...
        <div className="relative z-10">
          ...
          <PageHero
            eyebrow="Logistics Command Center · Live"
            title="Smart Routing"
            subtitle="Every returned item is automatically routed to its best next destination — reducing waste and recovering maximum value."
            visual={
              <GlassCard className="p-4 h-[280px] relative overflow-hidden" hover={false}>
                <svg viewBox="0 0 400 260" className="w-full h-full">
                  {connections.map(([a,b],i) => {
                    const n1=nodes[a], n2=nodes[b];
                    return <motion.line key={i} x1={n1.x} y1={n1.y} x2={n2.x} y2={n2.y}
                      stroke="#10b981" strokeWidth="1.5" strokeDasharray="6 4"
                  ...
                  {connections.map(([a,b],i) => {
                    const n1=nodes[a], n2=nodes[b];
                    return <motion.circle key={`d${i}`} r="3.5" fill="#FF9900"
                      initial={{cx:n1.x,cy:n1.y,opacity:0}}
                      animate={{cx:[n1.x,n2.x],cy:[n1.y,n2.y],opacity:[0,1,0]}}
                      transition={{duration:2.5,repeat:Infinity,delay:i*0.6,ease:"easeInOut"}} />;
                  })}
                  {nodes.map((n,i) => (
                    <g key={n.label}>
                      ...
                      <motion.circle cx={n.x} cy={n.y} r="14" fill="none" stroke="#FF9900" strokeWidth="1"
                        animate={{r:[14,22],opacity:[0.5,0]}} transition={{duration:2,repeat:Infinity,delay:i*0.3}} />
                      <text x={n.x} y={n.y+28} textAnchor="middle" fill="white" fontSize="9" opacity="0.7">{n.label}</text>
                    </g>
                  ))}
                </svg>
              </GlassCard>
          ...
          <AnimatedSection className="mt-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <GlassCard className="p-6" hover={false}>
                ...
                <div className="space-y-5">
                  <div>
                    <label className="block text-white/60 text-xs uppercase tracking-widest font-semibold mb-1.5">Product Name</label>
                  ...
                  <Slider label="Condition Score"      value={form.conditionScore}      onChange={set("conditionScore")}      unit="/100" />
                  <Slider label="Original Price"       value={form.originalPrice}       onChange={set("originalPrice")}       min={5} max={2000} step={5} unit=" $" />
                  <Slider label="Est. Repair Cost"     value={form.estimatedRepairCost} onChange={set("estimatedRepairCost")} min={0} max={500}  step={5} unit=" $" />
                  <div className="grid grid-cols-2 gap-4">
                    <Select label="Category"     value={form.category}    onChange={set("category")}    options={CATEGORIES}  />
                    <Select label="Demand Level" value={form.demandLevel} onChange={set("demandLevel")} options={DEMAND_OPTS} />
                  </div>
                  <div>
                    <label className="block text-white/60 text-xs uppercase tracking-widest font-semibold mb-1.5">Return Reason</label>
                    <input type="text" value={form.returnReason}
                      onChange={e => set("returnReason")(e.target.value)}
                      placeholder="e.g. Wrong size, Changed mind…"
                      className="w-full bg-white/5 border border-white/15 rounded-xl px-3 py-2.5 text-white
                        text-sm placeholder-white/25 focus:outline-none focus:border-[#FF9900]/50 transition-colors" />
                ...
                <motion.div className="mt-6" whileTap={{ scale: 0.98 }}>
                  <GlowButton onClick={runEngine} className="w-full" disabled={running}>
                    {running ? (
                      <span className="flex items-center gap-2">
                      ...
                      <span className="flex items-center gap-2"><span>⚡</span> Run Routing Engine</span>
                    )}
                  </GlowButton>
                </motion.div>
              </GlassCard>
                ...
                <AnimatePresence mode="wait">
                  {gradingResult ? (
                    ...
                    <motion.div key="placeholder"
                      ...
                      className="rounded-2xl border border-white/8 bg-white/[0.03] min-h-[300px]
                        flex flex-col items-center justify-center text-center p-8">
                      <motion.div
                        animate={{ rotate: [0,10,-10,0], scale: [1,1.05,1] }}
                        transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                      ...
                      <p className="text-white/50 text-sm">
                        Configure parameters and run the engine to see the recommended route with impact analysis.
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
          ...
          {/* ── Impact Insights Panel ── */}
          <AnimatedSection className="mt-10">
            <ImpactInsightsPanel decision={decision} />
          </AnimatedSection>
          ...
          <AnimatedSection className="mt-10">
            ...
            <div className="flex flex-col">
              ...
              <ProcessStepCard step={2} title="AI condition check"
                description="The AI Grading system assesses condition, completeness, and functionality." />
              <ProcessStepCard step={3} title="Routing decision + impact analysis"
                description="The routing engine selects the best route and calculates financial and sustainability impact." />
              <ProcessStepCard step={4} title="Outcome tracked"
                description="The outcome is logged in the item's Product Passport and counted toward sustainability savings."
                isLast />
            </div>
